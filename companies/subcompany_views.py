...
def subcompany_wizard_step4(request, company, context):
    ...
    if request.method == 'POST':
        ...
        # Récupérer les assignations depuis le formulaire
        for key, value in request.POST.items():
            if key.startswith('assignment_'):
                # Format: assignment_manager_id_subcompany_id
                parts = key.split('_')
                if len(parts) >= 4:
                    manager_id = parts[1]
                    subcompany_id = parts[2]
                    
                    if value == 'on':  # Checkbox cochée
                        assignments.append((manager_id, subcompany_id))
        
        try:
            with transaction.atomic():
                # Supprimer les assignations existantes pour cette entreprise
                SubCompanyUser.objects.filter(
                    subcompany__parent_company=company
                ).delete()
                
                # Créer les nouvelles assignations
                created_count = 0
                for manager_id, subcompany_id in assignments:
                    try:
                        company_user = CompanyUser.objects.get(id=manager_id, company=company)
                        subcompany = SubCompany.objects.get(id=subcompany_id, parent_company=company)
                        ...
                        # Définir la première sous-entreprise comme courante si pas déjà définie
                        if not company_user.current_subcompany:
                            company_user.current_subcompany = subcompany
                            company_user.save()
                        
                        created_count += 1
                        
                    except (CompanyUser.DoesNotExist, SubCompany.DoesNotExist):
                        continue
                
                messages.success(request, f"{created_count} assignation(s) créée(s) avec succès.")
    ...
    context['managers'] = company.company_users.filter(role='manager')
    context['subcompanies'] = company.subcompanies.all()
